    ...
    <div className="session-page">
      ...
      <div className="session-main">
        <div className="session-video-wrap">
          <video ref={videoRef} src={currentEx.videoUrl} style={cropStyle(currentEx.cropAspect, currentEx.cropX, currentEx.cropY, currentEx.cropW, currentEx.cropH)} className="session-video" muted playsInline preload="auto" />
          {phase === PHASE.INTRO && (
      ...
      <div className="session-queue">
        {routine.exercises.map((ex,i)=>(
          <div key={i} className={`queue-item ${i===exIdx?'active':i<exIdx?'done':''}`}>
            <div className="queue-thumb"><video src={ex.videoUrl} style={cropStyle(ex.cropAspect, ex.cropX, ex.cropY, ex.cropW, ex.cropH)} muted playsInline preload="auto" className="queue-video" onLoadedMetadata={e => { e.target.currentTime = ex.trimStart || 0 }} /></div>
