...
export async function detectScenes(videoUrl, options = {}) {
  const {
    sampleInterval = 0.3,     // seconds between samples (finer for short clips)
    threshold      = 0.10,    // luminance diff threshold (0–1), lower = more sensitive
    minDuration    = 1.5,     // minimum scene duration in seconds (exercises ~3-8s)
    onProgress,
  } = options
